...
import ir.neo.git_version.core.semanticVersionRegex
import org.jetbrains.changelog.Changelog

plugins {
    ir.neo.`git-version-plugin`
    /**
...
gitVersion {
    on {
        branch(".+") {
            val rawBranch = it.refInfo.shortenName
            val sanitizedBranch = rawBranch
                .replace(Regex("[^0-9A-Za-z-]"), "-")
                .trim('-')
                .ifBlank { "branch" }
            "$defaultSemVersion-$sanitizedBranch-snapshot"
        }
...
val createReleaseFolderForCi by tasks.registering {
    ...
    dependsOn("app:$createBinariesForCi")
